...
1. **AppID**：产品中心 → **APPID账号管理** → **我关联的APPID账号**
2. **MchID**：帐户中心 → **商户信息**
3. **API 密钥**：账户中心 → **API 安全** → **API 密钥**（32 字符）
4. **证书**：账户中心 → **API 安全** → **商户 API 证书** 下载
5. **微信支付公钥**：账户中心 → **API 安全** → **微信支付公钥** 获取
6. **微信支付公钥 ID**：账户中心 → **API 安全** → **微信支付公钥** 获取
